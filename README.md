...
开发地址通常为 http://127.0.0.1:5173 。启动前自动解析拍摄信息并复制原图（不做重新压缩）。修改原始照片后重新启动开发服务，或运行 `npm run prepare:assets`。
...
- `assets/images/Background/`：原始摄影图片，支持 JPG、JPEG、PNG、WebP，原图直接用于首页展示、不做重新压缩；没有照片时不显示摄影区域。
- `assets/images/avatar.png`：头像原图，构建时生成 WebP 并使用带内容哈希的地址，更新头像后避免沿用旧缓存。
- `scripts/prepare-assets.mjs`：将原始摄影图片按原路径复制到输出（首页直接引用，避免 WebP 二次编码造成的画质损失），并解析 EXIF 展示数据；头像仍生成 280px WebP。按源文件内容哈希缓存于 `.cache/prepare-assets/`（不提交 Git），源图未变化时跳过重新解析；替换或删除源图后自动更新缓存并清理不再引用的文件。
- `404.html`、`502.html`：原错误页源文件，构建时移除外部字体请求、并把本地化脚本地址改为自托管副本再复制到输出。`assets/vendor/error-pages-l10n.min.js` 存放该副本（源自 tarampampam/error-pages，MIT 许可），错误页不再在运行时请求第三方 CDN。
- `assets/js/`、`assets/css/`：保留作迁移参考，React 入口不再加载，也不复制到部署目录。

生成的 `public/assets/` 和 `src/generated/` 不提交 Git，每次生成前自动清理；不要在这些目录手工保存文件。源目录删除的图片不会残留在下次构建中。首次独立运行类型检查前需要 `npm run prepare:assets`。

日期原值 `11/12/2021 11:45:14` 现明确为 `2021-11-12T11:45:14+08:00`，按原 JavaScript 月／日解析习惯解释；如实际建站日期不同，在配置里修正。

## 本次交互与资源调整

- 图标随 npm 依赖构建，首页与错误页均不依赖第三方字体、图标或脚本 CDN。
- Vite 构建产物输出到 `/assets/build/`，与未哈希的历史 `/assets/icons/`、`/assets/images/` 分开，前者使用 `immutable` 长期缓存，后者保持 1 天缓存。
- 摄影背景直接使用原始 JPG，不再生成 WebP 缩略图；代价是首屏会加载完整原图（单张约 2.5–3.8 MB）且原图路径不带哈希、缓存 1 天。对体积敏感时可改为高质量／无损 WebP。
- 京华老宋体暂替换为系统宋体回退。恢复原字体前应核对字体许可并准备可自托管的字体文件。
- Discord 改为按钮，点击复制；结果由浏览器原生 `alert` 弹窗提示，不再在名片下方显示文字。
...
- 首页 JavaScript gzip 约 64.55 KB；背景图直接使用原始 JPG（单张约 2.5–3.8 MB，画质优先）。

## 静态站样式对齐

沿用当前依赖与系统宋体回退，不增加 CDN。按原站 CSS 恢复 hover／active 颜色 `#39c5bb` 和 0.5 秒过渡、两行菜单 15px／12px 间距和 18px 下边距、140px 头像、35px／24px 标题、100vh 内容区、移动端 90% 内容宽度与页脚 20px 10px 内边距、EXIF 左下角 50% 透明文字以及分阶段淡入动画。Discord 复制结果使用浏览器原生 `alert` 弹窗，不再占用名片下方空间。
